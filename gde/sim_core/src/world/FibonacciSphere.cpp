...
namespace {
	...
	const double k_golden_angle = 3.141592653589793 * (3.0 - Cmath::sqrt(5.0));
}

void FibonacciSphere::fill(size_t n, Vector<Vec3> &out) {
	out.clear();
	if (n == 0) return;
	...
	for (size_t i = 0; i < n; ++i) {
		double r = Cmath::sqrt(1.0 - z * z);
		double x = Cmath::cos(longitude) * r;
		double y = Cmath::sin(longitude) * r;
